...
`e2e/live.test.ts` calls the ChatGPT Codex backend and runs only when
`CODEX_LIVE_ACCESS_TOKEN` is set:

```sh
CODEX_LIVE_ACCESS_TOKEN=<access token> CODEX_LIVE_ACCOUNT_ID=<account id> \
bun run test:e2e
```

`CODEX_LIVE_MODEL` is optional (default `gpt-5.5`).
